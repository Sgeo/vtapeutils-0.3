...
int vtape_read(VTAPE_FILE *infile, unsigned char *buffer, unsigned int maxlen, PARITY *parity)
{
  int result;
  ...
  switch (infile->type) {
    ...
    case VTAPE_P7B: result=p7b_read(infile,buffer,maxlen,parity); break;
  }
  return result;
}
